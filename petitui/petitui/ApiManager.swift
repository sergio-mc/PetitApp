...
class ApiManager {
    ...
    static func createAnimal(pet:Pet,data:Data, completion: @escaping (Bool) -> ()){
     ...
     static func getFavoritesByUser( completion: @escaping ([Pet]) -> ()){
           let url = URL(string:"http://0.0.0.0:8888/petit-api/public/api/favorites")
           AF.request(url!,
                      method: .get
           )
               .validate()
    ...
    static func createFavorite(favoriteModel: FavoriteModel ,data:Data, completion: @escaping (Bool) -> ()){
         ...
         let url = "http://0.0.0.0:8888/petit-api/public/api/add/favorite"
         AF.upload(multipartFormData: { multipartFormData in
